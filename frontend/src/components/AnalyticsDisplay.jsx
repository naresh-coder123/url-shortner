...
export default function AnalyticsDisplay({ data = {} }) {
  ...
  const metrics = useMemo(() => {
    ...
    const todayCount = analytics.filter(
    ...
    const topDevice =
      Object.entries(deviceMap).sort((a, b) => b[1] - a[1])[0]?.[0] ?? "—";
  ...
  return (
    <div className="font-sans">
      ...
      <div className="mb-6">
        {longUrl && (
          <p className="text-sm text-yellow-900/50 truncate max-w-xl mb-1">
            {longUrl}
          </p>
        )}
        {shortUrl && (
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        ...
        <MetricCard
          label="Top device"
          value={
            metrics.topDevice.charAt(0).toUpperCase() +
            metrics.topDevice.slice(1)
          }
        />
      </div>
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
            <SectionCard title="Device breakdown">
              <Legend
                items={deviceData.map(({ name, value }, i) => ({
